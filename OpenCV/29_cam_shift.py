...
# CAM shift - Continuously Adoptive Meanshift
# Applies meanshift after it converges -> updates the size of the window
# Also, calculates the best fitting ellipse on it
...
while(1):
    ...
    if ret == True:
        ...
        ret, track_window = cv2.CamShift(dst, track_window, term_criteria) # Change meanShift() to CamShift()
        # (destination image, track image/window, termination criteria)
        #print(ret) 
        # ret contains values of ((x, y), (w, h), value_of_rotation)  
    
        # Draw it on an image
        pts = cv2.boxPoints(ret)
        #print(pts)
        pts = np.int0(pts) # Convert this array into int values from float
        # We will get a rotating rectangle, we cannot use cv2.rectangle
        final_image = cv2.polylines(frame, [pts], True, (0, 255, 0), 2)
        # (frame, pts value, closed=True, color, thickness)
